/* vR2 — Mobile/Tablet runtime only; Desktop intacto */
(function(){
  ...
  if(document.getElementById('r2-header')) return;

  const header = document.createElement('div'); header.id = 'r2-header';
  const brand = document.createElement('a'); brand.className = 'brand'; brand.href = 'index.html';
  const logo  = document.createElement('img'); logo.id = 'r2-logo'; logo.alt = 'Grupo AAA Asesores';
  ...
  const burger = document.createElement('button'); burger.id = 'r2-burger'; burger.setAttribute('aria-label','Abrir menú');
  const bar = document.createElement('span'); burger.appendChild(bar);

  header.appendChild(brand); header.appendChild(burger);
  body.prepend(header);
  body.classList.add('r2-has-header');

  const menu = document.createElement('nav'); menu.id = 'r2-menu';
  menu.setAttribute('role','menu'); menu.setAttribute('aria-label','Menú móvil');
  [['index.html','INICIO'],['productos.html','PRODUCTOS'],['servicios.html','SERVICIOS'],
   ['nosotros.html','NOSOTROS'],['casos-de-exito.html','CASOS DE ÉXITO'],['index.html#contacto','CONTÁCTENOS']]
  ...
  function syncHeader(){
    ...
    document.documentElement.style.setProperty('--r2h', h + 'px');
  ...
  function openMenu(){ menu.style.display='inline-block'; fitMenuWidth(); body.classList.add('r2-menu-open'); }
  function closeMenu(){ menu.style.display='none'; body.classList.remove('r2-menu-open'); }
  ...
  document.addEventListener('click', (e)=>{ if(e.target.closest('#r2-menu')||e.target.closest('#r2-burger')) return; closeMenu(); }, true);

  function scrollToModules(){
    const anchor = document.querySelector('#modulos, #modules');
    if(!anchor) return;
    const rect = anchor.getBoundingClientRect();
    const h = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--r2h')) || 72;
