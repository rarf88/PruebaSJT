...
/* responsive-v6.js — ULTRA mobile/tablet ONLY */
(function(){
  ...
  let raf = null;
  window.addEventListener('scroll', ()=>{ if(!raf){ raf = requestAnimationFrame(()=>{ setHeaderH(); raf=null; }); } }, {passive:true});
  ...
  Array.from(document.querySelectorAll('.mobile-menu-overlay, .menu-backdrop, .nav-overlay, .drawer-overlay, .backdrop, .overlay, .mask'))
    .forEach(el=>{ el.style.display='none'; el.style.pointerEvents='none'; });

  // ===== SLIDER (Slick): disable centerMode/padding =====
  try{ if(window.$ && typeof $==='function' && $('.slider').slick){
    $('.slider').slick('slickSetOption', { centerMode:false, centerPadding:'0px' }, true);
  } }catch(e){}

  // ===== PRODUCTOS: force "Ver módulos" to show modules & scroll =====
  (function(){
    ...
    function showModules(targetId){
      ...
      // aseguremos hash (por si otros scripts dependen de él)
      try{ history.replaceState(null, '', '#modulos'); }catch(e){}
    }

    // Captura clics incluso si cambian clases/markup
    ...
    // Neutraliza restauraciones a entidades
