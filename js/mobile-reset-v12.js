...
// v13.2 — targeted runtime fixes (mobile only)
(function(){
  if(!matchMedia('(max-width:991px)').matches) return;

  // 1) Enlazar burger -> toggle menú + bloquear scroll
  const menu   = document.getElementById('mobi-menu');
  const burger = document.getElementById('mobi-burger');
  function fitMenuWidth(){
    if(!menu) return;
  ...
  // 2) Asegurar links del menú — si no hay, construir por defecto
  ...
  // 3) Slider: si existe Slick, reducir aún más flechas; si no, ajustar controles genéricos
  try{
    if(window.$ && typeof $==='function' && $('.slider').slick){
      $('.slider').slick('slickSetOption',{ arrows:true, dots:true }, true);
      const w=24,h=24;
      document.querySelectorAll('.slick-prev, .slick-next').forEach(b=>{ b.style.width=w+'px'; b.style.height=h+'px'; });
    }
  }catch(e){}

  // 4) Quitar overlays que cubren menú antiguo
  document.querySelectorAll('.mobile-menu-overlay, .menu-backdrop, .nav-overlay, .drawer-overlay, .overlay, .mask, [class*="offcanvas-backdrop"]').forEach(el=>{
